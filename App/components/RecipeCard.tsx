...
const RecipeCard: React.FC<RecipeCardProps> = ({ recipe, onEdit, onView, onDelete }) => {
  const defaultImage = `https://picsum.photos/seed/${recipe.id || recipe.title}/400/300`;
    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow border border-gray-100 group">
      ...
      >
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          <span className="bg-white/90 backdrop-blur-sm text-xs font-semibold px-2 py-1 rounded-md text-gray-700 shadow-sm">
            {recipe.cuisineType}
          </span>
          <span className={`text-xs font-semibold px-2 py-1 rounded-md shadow-sm ${
      ...
      <div className="p-5">
        <h3 
          className="text-lg font-bold text-gray-900 line-clamp-1 mb-1 cursor-pointer hover:text-orange-600"
          onClick={() => onView(recipe)}
        >
          {recipe.title}
        </h3>
