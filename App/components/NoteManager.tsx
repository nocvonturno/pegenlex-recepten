...
const NoteManager: React.FC<NoteManagerProps> = ({
  ...
  const fetchNotes = async (currentSearch: string) => {
    setLoading(true);
    try {
      const response = currentSearch
        ? await apiService.searchNotes(currentSearch, { page: 0, size: 50 })
        : await apiService.getNotes({ page: 0, size: 50 });
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Bewaar je gedachten en ideeën.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full py-12 text-center text-gray-400 italic">Geen notities gevonden.</div>
        ) : (
          notes.map(note => (
            <div
              key={note.id}
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-bold text-gray-900 text-lg group-hover:text-orange-600 transition-colors">{note.title}</h3>
                  <p className="text-xs text-gray-400 mt-1">
                    {note.createdAt ? new Date(note.createdAt).toLocaleDateString('nl-NL') : 'Datum onbekend'}
                    {note.updatedAt && note.updatedAt !== note.createdAt && (
                      <span className="ml-2">(bewerkt {new Date(note.updatedAt).toLocaleDateString('nl-NL')})</span>
                    )}
                  </p>
                </div>
              </div>
              <p className="text-sm text-gray-600 line-clamp-3 mb-6 min-h-[40px]">
                {note.content ? truncateContent(note.content, 150) : 'Geen inhoud'}
              </p>
              ...
              <div className="flex justify-end gap-2 border-t pt-4">
                ...
                <button
                  onClick={(e) => note.id && handleDelete(e, note.id)}
